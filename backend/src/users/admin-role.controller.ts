...
export class AdminRoleController {
  ...
  async setRolePermissions(@Param('id') id: string, @Body() body: { permissionKeys: string[] }) {
    ...
    for (const key of body.permissionKeys) {
      ...
      if (perm) {
        const rp = this.rpRepo.create({ role: (await this.roleRepo.findOne({ where: { id } }))?.key || '', roleId: id, permissionId: perm.id });
